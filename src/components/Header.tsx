...
function Header() {
  ...
  const services = [
    { title: 'Tech Services', description: 'Comprehensive IT support for all needs.', icon: '🖥️' },
    { title: 'BPO Services', description: 'Enhancing BPO strategies to maximize revenue.', icon: '💼' },
    ...
    <header className="bg-lightBg dark:bg-darkBg dark:text-darkText shadow-md sticky top-0 z-50">
      <nav className="w-full px-6 flex justify-between items-center py-4">
        <div className="logo hidden md:block">
          <img
            src="https://derhjzjgqhzkqefjmrer.supabase.co/storage/v1/object/public/assets/logo.jpg"
            alt="AXIMO Logo"
            className="w-36"
          />
        </div>
        ...
        <ul className="hidden md:flex space-x-6 text-lightText dark:text-darkText items-center">
          ...
          {navItems.map((item) =>
            item === "Services" ? (
              <li
                key={item}
                className="relative group dark:hover:text-white font-medium"
              >
                <button
                ...
                {dropdownOpen && (
                  <ul className="absolute left-0 mt-2 w-48 bg-white dark:bg-darkBg shadow-lg">
                    { services.map((item) => 
                    <li>
                      <Link
                        to="/services/IT-"
                        className="block px-4 py-2 text-gray-700 dark:text-darkText dark:hover:text-lightText hover:bg-gray-100"
                      >
                        IT Services
                      </Link>
                    </li>
                    ...
                    <li>
                      <Link
                        to="/services/call-center-services"
                        className="block px-4 py-2 text-gray-700 dark:text-darkText dark:hover:text-lightText hover:bg-gray-100"
                      >
                       BPO Services
                      </Link>
      ...
      >
        ...
        {navItems.map((item) => (
          <li
            key={item}
            className="py-4 text-center dark:hover:text-white font-medium"
          >
            <Link
              to={`/${item.toLowerCase()}`}
